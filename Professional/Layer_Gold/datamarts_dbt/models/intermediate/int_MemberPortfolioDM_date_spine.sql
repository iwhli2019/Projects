...
all_dates as (
    select
        cast(start_date as date) as calendar_date
    from date_range

    union all

    select
        cast(strftime('%Y-%m-%d', date(calendar_date, '+1 day')) as date)
    from all_dates
...
select
    ...
    cast(strftime('%Y-%m-%d', date(month_start_date, '+1 month', '-1 day')) as date) as month_end_date
from monthly_spine
order by 1